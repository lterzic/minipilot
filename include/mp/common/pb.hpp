...
inline pblink::vector3f_s pb_from(const vector3f& v) noexcept
{
    return { v(0), v(1), v(2) };
}

inline vector3f pb_to(const pblink::vector3f_s& v) noexcept
{
    return { v.x, v.y, v.z };
}

inline pblink::quaternionf_s pb_from(const quaternionf& q) noexcept
{
    auto qv = q.as_vector();
    return { qv(0), qv(1), qv(2), qv(3) };
}

inline quaternionf pb_to(const pblink::quaternionf_s& q) noexcept
{
    return { q.w, q.x, q.y, q.z };
}

inline pblink::time_s pb_from(const milliseconds_t& t) noexcept
{
    return pblink::time_s {
