...
public:
     ...
     * @param thrust Thrust in the upwards direction in Newtons
     * @param torque Torque vector using forward-right-down coordinates in Newton-meters
     * @returns `false` if the given arguments are outside of the achievable range,
     * else sets the desired actuation and returns `true`
     */
    virtual bool actuate(float thrust, vector3f torque) noexcept = 0;
